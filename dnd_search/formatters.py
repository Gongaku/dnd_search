    ...
    for block in blocks:
        ...
        if btype == "paragraph":
            ...
            for item in block.get("items", []):
                if fmt == "markdown":
                    print(
                        textwrap.fill(f"- {item}", width=_WRAP, subsequent_indent="  ")
                    )
                else:
...
def format_class_header_panel(class_name: str, detail: Mapping[str, Any]) -> None:
    ...
    if hit_die or hp1 or hpN:
        ...
        if hpN:
            lines.append(
                f"  [bold yellow]Hit Points at Higher Levels:[/bold yellow] {hpN}"
            )
        ...
        for label, val in prof_lines:
            if label == "Skills":
                ...
                if " from " in val.lower():
                    ...
                    skills = [
                        s.strip().lstrip("and ").strip() for s in skill_str.split(",")
                    ]
                    lines.append(
                        f"  [bold yellow]{label}:[/bold yellow] {preamble} from:"
                    )
                    for skill in skills:
                        if skill:
...
def _format_spells_list(spells: list[Spell], fmt: _Fmt) -> None:
    ...
    print(
        _md_table(headers, rows) if fmt == "markdown" else _plain_table(headers, rows)
    )
...
def _format_class_detail(cls: DnDClass, detail: Mapping[str, Any], fmt: _Fmt) -> None:
    ...
    if detail.get("subclasses"):
        ...
        print(
            ...
            if fmt == "markdown"
            else _plain_table(headers, rows)
        )
